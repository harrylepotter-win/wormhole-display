...
/* OpenBSD does not have these options.  */
        ...
        if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
            ...
            if (payload == NULL && ret == 0) {
                ...
                /* Android port: EOF ends this stream, including its decoder state. */
                conn_reset = true;
                break;
            ...
            /* Android port: bound allocation before any payload parser runs. */
            if (payload_size < 0 || payload_size > 32 * 1024 * 1024) {
                conn_reset = true;
                break;
            }
            ...
            switch (packet[4]) {
            case  0x00:
                /* Android port: an AVCC unit needs a length and a NAL header. */
                if (payload_size < 5) { conn_reset = true; break; }
                ...
                while (nalu_size < payload_size) {
                    /* Android port: validate the length field BEFORE reading it. */
                ...
                if(!valid_data) {
                    ...
                    /* Android port: a lost reference picture needs session recovery. */
                    free(payload_out);
                    conn_reset = true;
                    break;
                }
            ...
            case 0x01:
                ...
                */
                ...
                prepend_sps_pps = false;  /* Android port: invalidate old config before replacing it. */
                sps_pps_len = 0;
                if (sps_pps) {
                    free(sps_pps);
                    sps_pps = NULL;
                }
                /* Android port: bounds validation. Reject packets too
                 * short for the fields read below instead of reading or
                 * copying out of the declared payload bounds. */
                if (payload_size < 8) {
                    logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
                               "raop_rtp_mirror: type 0x01 packet payload too short (%d bytes); dropped",
                               payload_size);
                    break;
                }
                ...
                if (!memcmp(payload + 4, hvc1, 4)) {
                    ...
                    int remaining = payload_size - 0x75;  /* Android port: bounds validation */
                    if (remaining < 5) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
                                   "raop_rtp_mirror: HEVC VPS/SPS/PPS payload truncated (%d bytes)", payload_size);
                        break;
                    }
                    ...
                    ptr += 5;
                    remaining -= 5;
                    if (vps_size < 0 || remaining < (int) vps_size + 5) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR, "raop_rtp_mirror: HEVC VPS truncated");
                        break;
                    }
                    ...
                    ptr += 5;
                    remaining -= 5;
                    if (sps_size < 0 || remaining < (int) sps_size + 5) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR, "raop_rtp_mirror: HEVC SPS truncated");
                        break;
                    }
                    ...
                    ptr += 5;
                    remaining -= 5;
                    if (pps_size < 0 || remaining < (int) pps_size) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR, "raop_rtp_mirror: HEVC PPS truncated");
                        break;
                    }
                    ...
                    /* Android port: bounds validation for H.264 SPS/PPS */
                    unsigned short sps_size = (unsigned short) byteutils_get_short_be(payload, 6);
                    if (payload_size < 8 + (int) sps_size + 3) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
                                   "raop_rtp_mirror: H.264 SPS truncated (%d bytes, sps_size=%u)",
                                   payload_size, sps_size);
                        break;
                    }
                    unsigned char *sequence_parameter_set = payload + 8;
                    unsigned short pps_size = (unsigned short) byteutils_get_short_be(payload, sps_size + 9);
                    if (payload_size < 11 + (int) sps_size + (int) pps_size) {
                        logger_log(raop_rtp_mirror->logger, LOGGER_ERR,
                                   "raop_rtp_mirror: H.264 PPS truncated (%d bytes, sps_size=%u, pps_size=%u)",
                                   payload_size, sps_size, pps_size);
                        break;
                    }
            ...
            case 0x05:
                ...
                if (payload_size) {
                    ...
                    if (plist_size) {
                        ...
                        plist_free(root_node); /* Android port: release each streaming report. */
                    }
                }
                break;
            default:
    ...
    /* Android port: every exit owns these allocations, including short reads. */
    free(payload);
    free(sps_pps);
    ...
    MUTEX_LOCK(raop_rtp_mirror->run_mutex);
    bool unexpected_exit = raop_rtp_mirror->running;
    ...
    if (unexpected_exit && raop_rtp_mirror->callbacks.conn_reset) {
        raop_rtp_mirror->callbacks.conn_reset(raop_rtp_mirror->callbacks.cls, 1);
    }

    if (unsupported_codec) {
        /* Android port: the controller joins/closes this worker; never self-join. */
...
void raop_rtp_mirror_stop(raop_rtp_mirror_t *raop_rtp_mirror) {
    ...
    MUTEX_LOCK(raop_rtp_mirror->run_mutex);
    /* Android port: running=false does not mean callbacks have finished. */
